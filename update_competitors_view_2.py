...
# I will write the complete content of the file.
new_file_content = """
"use client";
...
export default function CompetitorsView({ initialCompetitors }: CompetitorsViewProps) {
  ...
  // New state for Curcumin Checkboxes
  ...
  const toggleCurcuminFilter = (val: string) => {
    setSelectedCurcuminFilters(prev => 
        ...
        <div className="px-4 pb-3">
          {id === 'curcumin' ? (
            <div className="flex flex-col gap-1.5 max-h-[200px] overflow-y-auto scrollbar-hide pr-2">
              {curcuminOptions.map(opt => (
                <label key={opt} className="flex items-center gap-2 cursor-pointer group">
                  <div className={`w-3.5 h-3.5 rounded-sm border flex items-center justify-center transition-colors ${selectedCurcuminFilters.includes(opt) ? 'bg-[#F16775] border-[#F16775]' : 'border-[#2D3142]/20 group-hover:border-[#F16775]/50 bg-white'}`}>
                    {selectedCurcuminFilters.includes(opt) && <CheckSquare className="w-3 h-3 text-white" strokeWidth={3} />}
                  </div>
                  <span className={`text-xs ${selectedCurcuminFilters.includes(opt) ? 'text-[#2D3142] font-medium' : 'text-[#2D3142]/60 group-hover:text-[#2D3142]/80'}`}>
                    {opt}
                  </span>
                  <input 
                    type="checkbox" 
                    className="hidden" 
                    checked={selectedCurcuminFilters.includes(opt)}
                    onChange={() => toggleCurcuminFilter(opt)}
                  />
                </label>
              ))}
            </div>
...
357:           </div>
358: 
359:           <div className="p-4 border-t border-[#2D3142]/10 bg-[#F4F1EA] flex items-center justify-between text-xs text-[#2D3142]/50">
360:             <span>1 - {sortedCompetitors.length} of {sortedCompetitors.length}</span>
361:             <div className="flex gap-2">
362:               <button className="px-3 py-1 border border-[#2D3142]/10 rounded hover:bg-[#2D3142]/5">Previous</button>
363:               <button className="px-3 py-1 border border-[#2D3142]/10 rounded hover:bg-[#2D3142]/5 bg-white/[0.02]">1</button>
364:               <button className="px-3 py-1 border border-[#2D3142]/10 rounded hover:bg-[#2D3142]/5">Next</button>
365:             </div>
366:           </div>
367: 
368:         </div>
...
370:       </div>
371: 
372:       {selectedId && selectedCompetitor && (
373:         <CompetitorDossier 
374:           competitor={selectedCompetitor} 
375:           onClose={() => setSelectedId(null)} 
376:         />
377:       )}
378:     </div>
379:   );
...
print("Updated CompetitorsView.tsx with Checkboxes")
