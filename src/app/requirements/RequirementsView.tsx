...
export default function RequirementsView() {
  ...
  const certifications = [
    {
      ...
      theme: "bg-blue-500/5 border-blue-500/20 text-blue-400",
      tests: [
    ...
    },
    {
      ...
      icon: <FileBadge className="w-5 h-5 text-indigo-400" />,
      theme: "bg-indigo-500/5 border-indigo-500/20 text-indigo-400",
      tests: [
    ...
    },
    {
      ...
      theme: "bg-emerald-500/5 border-emerald-500/20 text-emerald-400",
      tests: [
    ...
    },
    {
      ...
      theme: "bg-amber-500/5 border-amber-500/20 text-amber-400",
      tests: [
  ...
  return (
    <div className="h-full bg-[#050505] p-8 overflow-y-auto space-y-8">
      {/* Page Header */}
      <div className="flex items-center gap-4 mb-2">
        ...
        <div>
          <h1 className="text-3xl font-light tracking-tight text-white/90">Requirements & Standards</h1>
          <p className="text-white/40 mt-1">Track and manage mandatory world-class tests and export registrations for Paradise Organics.</p>
        </div>
      </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
        <button 
          ...
          className={`w-full flex items-center justify-between p-6 bg-white/5 hover:bg-white/10 transition-colors text-left ${isGlobalOpen ? 'border-b border-white/10' : ''}`}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-medium text-white/90">Global Certifications & Standards</h2>
              <p className="text-sm text-white/40">Requirements for exporting into USA, EU, Middle East, and Australia.</p>
            </div>
          </div>
          {isGlobalOpen ? <ChevronUp className="text-white/40" /> : <ChevronDown className="text-white/40" />}
        </button>
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              {certifications.map((region) => (
                <div key={region.id} className="bg-black/20 border border-white/5 rounded-2xl overflow-hidden flex flex-col">
                  <div className={`px-5 py-4 border-b border-white/5 flex items-center gap-3 ${region.theme.split(' ')[0]}`}>
                    <div className="p-1.5 rounded-lg bg-black/20">
                      {region.icon}
                    </div>
                    <h3 className="text-lg font-medium text-white/90">{region.region}</h3>
                  </div>
                  
                  <div className="p-5 flex-1 flex flex-col gap-3">
                    {region.tests.map((test, i) => (
                      <div key={i} className="bg-white/5 border border-white/10 rounded-xl p-4 hover:border-white/20 transition-all group flex flex-col">
                        <div className="flex justify-between items-start mb-2">
                          <h4 className="text-white/90 font-medium text-sm">{test.name}</h4>
                        ...
                        <p className="text-xs text-white/50 mb-4 flex-1 leading-relaxed">{test.description}</p>
                        <div className="pt-3 border-t border-white/5">
                          <Link href={test.url} target="_blank" className="flex items-center gap-1.5 text-xs font-medium text-indigo-400 hover:text-indigo-300 transition-colors w-max">
            ...
            <div className="mt-6 bg-amber-500/5 border border-amber-500/20 rounded-2xl p-5 flex items-start gap-4">
              ...
              <div>
                <h3 className="text-amber-400 font-medium text-sm mb-1">Paradise Organics Quality Promise</h3>
                <p className="text-white/60 text-xs leading-relaxed max-w-4xl">
                  By fulfilling all these tests simultaneously, Paradise Organics will establish a globally unique moat. The certifications above cover heavy metals (Eurofins), biosecurity (BICON), chemical safety (EPA/REACH), and regional import laws (SFDA, FSMA)—making our supply chain universally compliant.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden shadow-2xl">
        <button 
          ...
          className={`w-full flex items-center justify-between p-6 bg-white/5 hover:bg-white/10 transition-colors text-left ${isRegistrationOpen ? 'border-b border-white/10' : ''}`}
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-medium text-white/90">Company Registration & Export Licenses</h2>
              <p className="text-sm text-white/40">Mandatory licenses for exporting from India to the world.</p>
            </div>
          </div>
          {isRegistrationOpen ? <ChevronUp className="text-white/40" /> : <ChevronDown className="text-white/40" />}
        </button>
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {registrations.map((reg) => (
                <div key={reg.id} className="bg-black/20 border border-white/5 rounded-2xl p-5 hover:border-white/10 transition-all flex flex-col group">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg border ${reg.theme}`}>
                        {reg.icon}
                      </div>
                      <h3 className="text-white/90 font-medium">{reg.name}</h3>
                    </div>
                  ...
                  <p className="text-xs text-white/50 mb-5 flex-1 leading-relaxed">{reg.description}</p>
                  
                  <div className="pt-4 border-t border-white/5 mt-auto">
                    <Link href={reg.url} target="_blank" className="flex items-center gap-1.5 text-xs font-medium text-emerald-400 hover:text-emerald-300 transition-colors w-max">
