...
async function scrapeCompany(buyerId: string, url: string, name: string) {
  ...
  try {
    ...
    const addressRegex = /\\b(?:Unit|Building|Street|Road|Avenue|Lane|Way|Park|Square|Estate)[\\s\\S]{1,100}?(?:[A-Z]{1,2}\\d{1,2}[A-Z]? \\d[A-Z]{2})\\b/g;
...
async function main() {
  ...
  const buyers = await prisma.buyer.findMany({
    where: { countryId: ukCountry.id },
    include: { websites: true }
  });

  console.log(`Processing ${buyers.length} UK companies...`);
  ...
  console.log("Finished scraping all UK companies.");
}
