...
async function scrapeCompany(buyerId: string, url: string, name: string) {
  ...
  try {
    ...
    // More generic regex for Europe
    const addressRegex = /\\b(?:Unit|Building|Street|Road|Avenue|Lane|Way|Park|Square|Estate|Al|Rue|Via|Strasse|Avenida)[\\s\\S]{1,100}?(?:London|Paris|Berlin|Rome|Madrid|Amsterdam|Vienna|Zurich|Geneva|Brussels|Europe)\\b/gi;
...
async function main() {
  const europeanCountriesRecords = await prisma.country.findMany({
    where: { name: { in: EUROPEAN_COUNTRIES } }
  });
  
  if (europeanCountriesRecords.length === 0) {
    console.log("Could not find any European countries in db");
    return;
  }

  const countryIds = europeanCountriesRecords.map(c => c.id);
  ...
  console.log(`Processing ${buyers.length} European companies...`);
  ...
  console.log("Finished scraping all European companies.");
}
